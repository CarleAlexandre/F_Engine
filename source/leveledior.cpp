...
//		if (ctx.tileset_ok) {
//			if (IsMouseInBound(ctx.tile_bound, {0, 20}, mouse_pos) && !ctx.file_active) {
...
//		//}
//		GuiScrollPanel(ctx.layer_bound, NULL, {ctx.layer_bound.x, ctx.layer_bound.y, ctx.layer_bound.width - 20, (float)(level.dimension.y * 20)}, &ctx.layer_scroll, &ctx.layer_view);
...
//					case (0): {
//						if (IsInBond(block_pos, {level.dimension.x - 1, level.dimension.z - 1}, {0, 0})) {
//							level.terrain[linearIndexFromCoordinate({block_pos.x, block_pos.y, (float)ctx.selected_layer}, level.dimension.x, level.dimension.z)] = ctx.selected_tile;
//						}
//						break;
//					}
//					case (1): {
//							level.terrain[linearIndexFromCoordinate({block_pos.x, block_pos.y, (float)ctx.selected_layer}, level.dimension.x, level.dimension.z)] = 0;
//						break;
//					}
//					case (2): {
//						if (ctx.selected_tile != -1) {
//							int y = ctx.selected_layer;
//							int beg = linearIndexFromCoordinate({0, 0, (float)y}, level.dimension.x, level.dimension.z);
//							int end = linearIndexFromCoordinate({level.dimension.y, level.dimension.z, (float)y}, level.dimension.x, level.dimension.z);
//							for (int idx = beg ; idx < end; idx++) {
//								level.terrain[idx] = ctx.selected_tile;
//							}
//						}
//						break;
//					}
...
//			ShowCursor();
//		}
//		GuiScrollPanel({ctx.tile_bound.width, 20, width - ctx.tile_bound.width, (float)GetScreenHeight() - 20}, NULL, {0, 0, (level.dimension.x + 2) * 32, (level.dimension.z + 2) * 32}, &ctx.draw_scroll, &ctx.draw_view);

//		Vector2 start = {0, 0};
//		start.x -= ctx.draw_scroll.x;
//		start.y -= ctx.draw_scroll.y;
//		start.y = floor(start.y / 32);
//		start.x = floor(start.x / 32);

//		Vector2 finish = {width - ctx.tile_bound.width, (float)GetScreenHeight() - 20};
//		finish.x -= ctx.tile_bound.width + 32;
//		finish.y -= 20 + 32;
//		finish.x -= ctx.draw_scroll.x;
//		finish.y -= ctx.draw_scroll.y;
//		finish.y = floor(finish.y / 32);
//		finish.x = floor(finish.x / 32);
...
//					int beg = linearIndexFromCoordinate({start.x, start.y, (float)y}, level.dimension.x, level.dimension.z);
//					int end = linearIndexFromCoordinate({finish.x, finish.y, (float)y}, level.dimension.x, level.dimension.z);
//					for (int idx = beg ; idx < end; idx++) {
//						DrawTextureRec(current_tileset, getTextureRec(level.terrain[idx], current_tileset), {ctx.draw_view.x + 32 + getXpos3d(idx, level.dimension.x) * 32 + ctx.draw_scroll.x, ctx.draw_view.y + 32 + getYpos3d(idx, level.dimension.x,  level.dimension.z) * 32 + ctx.draw_scroll.y}, WHITE);
//					}
//					DrawText(TextFormat("beg: %5i, end: %5i", beg, end), ctx.tile_bound.width, (float)40 + 20 * y, 16, WHITE);
//				}
...
//		}
//		if (GuiDropdownBox({200, 0, 100, 20}, "TERRAIN;WALL;EVENT", &ctx.tool_action, ctx.tool_active))
...
//					ctx.file_open = false;
//					openFile(&level, TextFormat("assets/maps/%s.map", ctx.filename));
//					ctx.draw_ready = true;
//				} else {
...
//			GuiTextBox({ctx.file_bound.x + 30, ctx.file_bound.y + 90, 50, 16}, ctx.dim_x, 4, dimx);
//			GuiTextBox({ctx.file_bound.x + 90, ctx.file_bound.y + 90, 50, 16}, ctx.dim_z, 4, dimz);
...
//				} else {
//					level = CreateNewLevel(dim, ctx.filename, ctx.tileset);
//					ctx.new_file = false;
//					ctx.draw_ready = true;
//				}
//			}
